...
// ─── Fee Fraction Calculation ────────────────────────────────

/**
 * Compute the creator's fraction of the total pool fee.
 * Mirrors src/lib/meteora/claim-fees.ts → creatorFraction()
 *
 ...
 *
 *   0% Tax: feeClaimer gets 0.8%. Creator gets 50% (0.4% of 0.8%).
 */
function creatorFraction(creatorFeePercent: number): number {
  if (creatorFeePercent <= 0) return 0.5;
...
export async function claimCreatorFees(
  ...
  // 2. Build the Meteora claim transaction
  const MAX_U64 = new BN('18446744073709551615');
  const sdkCreatorProp = params.poolCreator
    ? new PublicKey(params.poolCreator)
    : wallet.publicKey;
  ...
  const platformFraction = creatorFeePercent > 0
    ? (0.4 / (creatorFeePercent + 0.8))
    : 0.5;
