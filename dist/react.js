...
__export(react_exports, {
  ...
  TokenaProvider: () => TokenaProvider,
  useAxolotl: () => useAxolotl,
  useAxolotlWallet: () => useAxolotlWallet,
  useClaimFees: () => useClaimFees,
  useCreateToken: () => useCreateToken,
  useCreatorProfile: () => useCreatorProfile,
  useLifecycle: () => useLifecycle,
  usePoolState: () => usePoolState,
  useQuoteBuy: () => useQuoteBuy,
  useQuoteSell: () => useQuoteSell,
  useTokenBalance: () => useTokenBalance,
  useTokenList: () => useTokenList,
  useTokenSearch: () => useTokenSearch,
  useTokenState: () => useTokenState,
  useTokena: () => useTokena,
  useTokenaWallet: () => useTokenaWallet,
  useTrade: () => useTrade,
...
var TokenaContext = (0, import_react.createContext)(null);
function TokenaProvider({
  children,
  ...config
}) {
  const sdk = (0, import_react.useMemo)(() => new Tokena(config), [config.chainKey, config.factoryAddress]);
  const value = (0, import_react.useMemo)(
    () => ({ sdk, chainKey: config.chainKey }),
    [sdk, config.chainKey]
  );
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TokenaContext.Provider, { value, children });
}
function useTokena() {
  const ctx = (0, import_react.useContext)(TokenaContext);
  if (!ctx) {
...
function useTokenaWallet(targetChain) {
  ...
  const fetchBalance = (0, import_react2.useCallback)(async (provider, addr) => {
    try {
      const bal = await provider.getBalance(addr);
      return (0, import_ethers12.formatEther)(bal);
    } catch {
      return null;
    }
  }, []);
    ...
    async function checkConnection() {
      try {
        ...
        if (accounts.length > 0) {
          const provider = new import_ethers12.BrowserProvider(ethereum);
  ...
  const getSigner = (0, import_react2.useCallback)(async () => {
    const ethereum = getEthereum();
...
function useTrade(chainKey) {
  ...
  const sell2 = (0, import_react4.useCallback)(
    async (params, signer) => {
      setLoading(true);
      setError(null);
      try {
...
function useClaimFees(tokenAddresses, walletAddress, chainKey) {
  const sdk = useTokena();
  const [claimableMap, setClaimableMap] = (0, import_react5.useState)({});
  const [totalClaimedMap, setTotalClaimedMap] = (0, import_react5.useState)({});
  const [loading, setLoading] = (0, import_react5.useState)(true);
  const [claimingToken, setClaimingToken] = (0, import_react5.useState)(null);
  ...
  const claim = (0, import_react5.useCallback)(
...
function useTokenState(tokenAddress, options = {}) {
  ...
  const [enriched, setEnriched] = (0, import_react6.useState)(null);
...
function useQuoteBuy(tokenAddress, ethAmount, options = {}) {
  ...
  const timerRef = (0, import_react7.useRef)();
...
function useTradeHistory(tokenAddress, options = {}) {
  ...
  const [trades, setTrades] = (0, import_react9.useState)([]);
...
function useCreateToken(chainKey) {
  ...
  const createToken2 = (0, import_react10.useCallback)(
    async (params, signer) => {
      setLoading(true);
      setError(null);
      setResult(null);
...
function useTokenList(options = {}) {
  ...
  const [tokens, setTokens] = (0, import_react12.useState)([]);
  const [total, setTotal] = (0, import_react12.useState)(0);
  const [isLoading, setIsLoading] = (0, import_react12.useState)(true);
  const [error, setError] = (0, import_react12.useState)(null);
  const [currentOffset, setCurrentOffset] = (0, import_react12.useState)(offset);
...
function useLifecycle(tokenAddress, options = {}) {
  const sdk = useTokena();
  const { pollInterval = 15e3, chainKey } = options;
  const [lifecycle, setLifecycle] = (0, import_react15.useState)(null);
